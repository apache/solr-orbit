...
"""
The scheduler module defines an API to determine *when* ASB should issue a request. There are two types of schedulers:
...
return the next point in time when ASB should issue a request.
...
* ``before_request`` is invoked by ASB before a request is executed. ``now`` provides the current timestamp in seconds.
* Similarly, ``after_request`` is invoked by ASB after a response has been received. ``now`` is the current timestamp,
  ``weight``, ``unit`` and ``request_meta_data`` are passed from the respective runner. For a bulk request, ASB
...
If the scheduler also needs access to the parameter source, provide a ``parameter_source`` property. ASB injects the
task's parameter source into this property.
"""


def scheduler_for(task: solrorbit.workload.Task):
    """
