...
def install_default_log_config():
    ...
    stale = io.exists(log_config) and "osbenchmark" in open(log_config, encoding="UTF-8").read()
    if not io.exists(log_config) or stale:
...
def load_configuration():
    ...
    with open(log_config_path()) as f:
        # print("LOGGING LOAD CONFIG: ", json.load(f))
        return json.load(f)
...
def configure_logging():
    ...
    #
    # ---------------------------------------------------------------------------------------------
    # "solrorbit/worker_coordinator/worker_coordinator.py", line 220, in create_client
